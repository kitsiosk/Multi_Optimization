...
while norm(grad(xk, yk)) > epsilon
    ...
    hessk = vpa(hess(xk, yk), 5);
    gradk = vpa(grad(xk, yk), 5);
    eigenvalues = eig(hessk);
    max_eig = max(eigenvalues);
    % Add the max eigenvalue of the Hessian to its diagonal
    % in order to convert it into positive definite
    hessk = hessk + eye(2) * (max_eig + 0.1);
    eigenvalues = eig(hessk);   % Update the eigenvalues
    if ~(all(eigenvalues > 0) || all(eigenvalues < 0))
        fprintf('Hessian not positive definite. Newton method cannot apply\n');
        return;
    end
    if det(hessk) == 0
